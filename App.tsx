...
const App: React.FC = () => {
            ...
            if (storedUser) {
              ...
              if (user.role === Role.CANDIDATE) {
                ...
                if (data.reports && data.reports[reportKey]) {
                  ...
                  // Randomly select 2 tasks
                  const shuffled = [...simulation.tasks].sort(() => 0.5 - Math.random());
                  const selectedTasks = shuffled.slice(0, 2);
                  const candidateSimulation: Simulation = {
                      ...simulation,
                      tasks: selectedTasks
                  };
                  setActiveSimulation(candidateSimulation);
    ...
    if (user.role === Role.RECRUITER) {
      ...
      if (simIdFromUrl && allSimulations[simIdFromUrl]) {
        ...
        if (allReports[reportKey]) {
          ...
          const simulation = allSimulations[simIdFromUrl];
          // Randomly select 2 tasks
          const shuffled = [...simulation.tasks].sort(() => 0.5 - Math.random());
          const selectedTasks = shuffled.slice(0, 2);
          const candidateSimulation: Simulation = {
  ...
  const confirmAndStartSimulation = () => {
    ...
    if (simulation) {
        // Randomly select 2 tasks from the 10 generated
        const shuffled = [...simulation.tasks].sort(() => 0.5 - Math.random());
        const selectedTasks = shuffled.slice(0, 2);
