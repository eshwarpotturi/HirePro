...
app.get('/api/external/simulation-results/:id', (req, res) => {
    ...
    const results = Object.values(appData.reports)
        .filter(report => report.simulationId === id)
        .map(report => ({
            ...
            feedback: report.recommendationReasoning,
            scores: {
                stressManagement: report.stressManagementScore,
    ...
    res.json({
        ...
        recruiterEmail: simulation.recruiterEmail,
...
app.post('/api/generate-tasks', async (req, res) => {
    ...
    const prompt = `Based on the following job role, generate 10 realistic and distinct tasks that a candidate would perform during a work simulation. All tasks should be of similar difficulty. The tasks should test a range of skills. Critically, you MUST include a variety of task types. The available types are 'TEXT', 'IMAGE', 'AUDIO', and 'VIDEO'.
...
app.post('/api/suggest-criteria', async (req, res) => {
    ...
    await handleTextApiCall(res, (genAI) => genAI.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt }));
});
